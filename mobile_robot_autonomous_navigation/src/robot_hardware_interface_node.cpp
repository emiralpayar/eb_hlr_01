...
void ROBOTHardwareInterface::read() {
    ...
    // Only right motor read
    right_motor.readBytes(rbuff,1);
    x=(int8_t)rbuff[0];
    right_motor_pos+=angles::from_degrees((double)x);
    joint_position_[0]=right_motor_pos; // Adjusted index for right motor
}
...
void ROBOTHardwareInterface::write(ros::Duration elapsed_time) {
    ...
    // Only right motor write
    velocity=(int)angles::to_degrees(joint_velocity_command_[0]); // Adjusted index for right motor
    wbuff[0]=velocity;
    wbuff[1]=velocity >> 8;
